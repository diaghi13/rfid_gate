...
#
# Uso:
#   curl -fsSL https://raw.githubusercontent.com/diaghi13/rfid_gate/refactor-modular-architecture/fix_relay_await_bug_v2.sh | sudo bash
...
if [ "$EUID" -ne 0 ]; then
    echo -e "${RED}Errore: Eseguire come root: sudo bash fix_relay_await_bug.sh${NC}"
    exit 1
fi
...
backup_file() {
    ...
    if [ -f "$file_path" ]; then
        ...
        echo -e "  Backup creato: $backup_path"
    fi
}
...
main() {
    ...
    echo "  Sistema rileva automaticamente se patch e' attivo"
