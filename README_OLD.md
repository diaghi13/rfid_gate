...
1. **Setup iniziale**:
   ...
   ```bash
   cp .env.example .env
   nano .env  # Configura credenziali MQTT (GPIO già configurato con valori legacy)
   ```

   Il file `.env.example` include già la configurazione GPIO del sistema legacy testato.

2. **Configurazione GPIO (Sistema Legacy Testato)**:

   Il sistema usa di default la configurazione GPIO del sistema legacy **funzionante**:

   ```bash
   # Lettore IN (PN532 I2C)
   RFID_IN_RST_PIN=22      # Pin Reset
   RFID_IN_SDA_PIN=8       # Pin SDA/CS

   # Lettore OUT (PN532 SPI)
   RFID_OUT_RST_PIN=25     # Pin Reset
   RFID_OUT_SDA_PIN=7      # Pin CS (Chip Select)
   ```

   **📌 Pin Mapping Raspberry Pi**:

   - Pin 7 (GPIO 4) = CS per lettore OUT
   - Pin 8 (GPIO 14) = SDA per lettore IN
   - Pin 22 (GPIO 25) = RST per lettore IN
   - Pin 25 (GPIO 6) = RST per lettore OUT

3. **Avvia sistema principale**:
