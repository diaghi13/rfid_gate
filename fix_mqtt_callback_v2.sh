#!/bin/bash
# ===========================================
# Fix MQTT Callback Thread-Safe - v2
# ===========================================
#
# Fix rapido per l'errore RuntimeWarning nei callback MQTT.
#
# Uso:
#   curl -fsSL https://raw.githubusercontent.com/diaghi13/rfid_gate/refactor-modular-architecture/fix_mqtt_callback_v2.sh | sudo bash
...
if [ "$EUID" -ne 0 ]; then
    echo -e "${RED}Errore: Eseguire come root${NC}"
    exit 1
fi
...
update_repo() {
    echo -e "${YELLOW}Aggiorno repository...${NC}"
    ...
    if [ -d "$REPO_DIR" ]; then
        ...
        echo -e "${RED}Repository non trovato${NC}"
        exit 1
    fi
}
...
apply_fix() {
    ...
    if [ -f "$source_file" ]; then
        ...
        echo -e "${RED}File sorgente non trovato${NC}"
        exit 1
    fi
}
